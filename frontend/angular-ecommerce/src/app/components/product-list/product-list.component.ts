...
export class ProductListComponent implements OnInit {
  ...
  processResult() {
    return (data: { _embedded: { products: Product[]; }; page: { number: number; size: number; totalElements: number; }; }) => {
      //the LHS is properties defined in this class, RHS is data from Spring Data REST JSON
      this.products = data._embedded.products;
      this.thePageNumber = data.page.number + 1; //Spring Data REST: page are 0 based
      this.thePageSize = data.page.size;
      this.theTotalElements = data.page.totalElements;
    }
  }

  updatePageSize(pageSize: number & { target: HTMLInputElement }) {
